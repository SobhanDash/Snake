...
class snake(object):
    ...
    def __init__(self, color, pos):
        ...
        self.dirnx = 0                              #direction for x for moving of snake
        ...
        for i, c in enumerate(self.body):                                       # Loop through every cube in our body
            ...
            if p in self.turns:                                                 # If the cubes current position is one where we turned
                ...
                if c.dirnx == -1 and c.pos[0] <= 0:
                    c.pos = (c.rows - 1, c.pos[1])                              #if going left but reached edge of screen then appear at right
                elif c.dirnx == 1 and c.pos[0] >= c.rows - 1:
                    c.pos = (0, c.pos[1])                                       #if going right but reached edge of screen then appear at left
                elif c.dirny == 1 and c.pos[1] >= c.rows - 1:
                    c.pos = (c.pos[0], 0)                                       #if going down but reached edge of screen then appear at up
                elif c.dirny == -1 and c.pos[1] <= 0:
                    c.pos = (c.pos[0], c.rows - 1)                              #if going up but reached edge of screen then appear at down
                else:
    ...
    def addCube(self):
        ...
        if dx == 1 and dy == 0:                                                             #if moving right
            self.body.append(cube((tail.pos[0] - 1, tail.pos[1])))                          #add to the left side
        elif dx == -1 and dy == 0:                                                          #if moving left
            self.body.append(cube((tail.pos[0] + 1, tail.pos[1])))                          #add to the right
        elif dx == 0 and dy == 1:                                                           #if moving up
            self.body.append(cube((tail.pos[0], tail.pos[1] - 1)))                          #add below
        elif dx == 0 and dy == -1:                                                          #if moving down
            self.body.append(cube((tail.pos[0], tail.pos[1] + 1)))                          #add to the top
...
def main():
    ...
    while flag:
        ...
        clock.tick(8)                                                   # Will ensure our game runs at 8 FPS
        s.move()
